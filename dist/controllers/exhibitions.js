...
const addArtworkToExhibition = async (req, res, next) => {
    try {
        const { exhibition_id } = req.params;
        const userId = req.user?.userId;
...
const removeArtworkFromExhibition = async (req, res, next) => {
    try {
...
const updateArtworkInExhibition = async (req, res, next) => {
    try {
        const { exhibition_id, artwork_id } = req.params;
        ...
        const artworkIndex = exhibition.artworks.findIndex((artwork) => artwork.artworkId === artwork_id);
        if (artworkIndex === -1) {
        ...
        if (position !== undefined)
            exhibition.artworks[artworkIndex].position = position;
        if (notes !== undefined)
            exhibition.artworks[artworkIndex].notes = notes;
        if (curatorNotes !== undefined)
            exhibition.artworks[artworkIndex].curatorNotes = curatorNotes;
        if (displayOrder !== undefined)
            exhibition.artworks[artworkIndex].displayOrder = displayOrder;
...
exports.updateArtworkInExhibition = updateArtworkInExhibition;
const getPublicExhibitions = async (req, res, next) => {
    try {
        const { page = '1', limit = '20', theme, search, sortBy = 'createdAt', sortOrder = 'desc' } = req.query;
...
exports.getPublicExhibitions = getPublicExhibitions;
const getSharedExhibition = async (req, res, next) => {
    try {
...
exports.getSharedExhibition = getSharedExhibition;
const shareExhibition = async (req, res, next) => {
    try {
        const { exhibition_id } = req.params;
        const userId = req.user?.userId;
        if (!userId) {
...
exports.shareExhibition = shareExhibition;
const unshareExhibition = async (req, res, next) => {
    try {
        const { exhibition_id } = req.params;
        const userId = req.user?.userId;
        if (!userId) {
...
const searchExhibitions = async (req, res, next) => {
    try {
        ...
        if (curator && typeof curator === 'string') {
            exhibitions = exhibitions.filter((exhibition) => exhibition.curator?.username?.toLowerCase().includes(curator.toLowerCase()));
        }
        // Sort exhibitions
        exhibitions.sort((a, b) => {
            let aValue, bValue;
        ...
        // Format exhibitions for response
        const formattedExhibitions = paginatedExhibitions.map((exhibition) => ({
            id: exhibition._id,
            title: exhibition.title,
            description: exhibition.description,
            theme: exhibition.theme,
...
const getFeaturedExhibitions = async (req, res, next) => {
    try {
        ...
        const featuredExhibitions = exhibitions
            ...
            const hasGoodTags = exhibition.tags?.some((tag) => ['featured', 'curated', 'highlight', 'masterpiece', 'collection'].includes(tag.toLowerCase()));
            ...
            .slice(0, limitNum);
        // Format exhibitions for response
        const formattedExhibitions = featuredExhibitions.map((exhibition) => ({
            id: exhibition._id,
            title: exhibition.title,
            description: exhibition.description,
            theme: exhibition.theme,
...
exports.getFeaturedExhibitions = getFeaturedExhibitions;
const getTrendingExhibitions = async (req, res, next) => {
    try {
        ...
        const trendingExhibitions = exhibitions
            ...
            .slice(0, limitNum);
        // Format exhibitions for response
        const formattedExhibitions = trendingExhibitions.map((exhibition) => ({
            id: exhibition._id,
            title: exhibition.title,
            description: exhibition.description,
            theme: exhibition.theme,
...
exports.getTrendingExhibitions = getTrendingExhibitions;
