...
exports.searchRijksByTitleOrArtist = exports.getVAArtwork = exports.searchVAByTitleOrArtist = exports.searchVAMuseum = exports.searchMetByTitleOrArtist = exports.getRijksArtwork = exports.searchRijksmuseum = exports.getMetArtwork = exports.searchMetMuseum = exports.getDepartments = exports.getRandomArtworks = exports.getArtworkById = exports.searchByTitleOrArtist = exports.searchArtworks = void 0;
const metMuseum_1 = require("../services/metMuseum");
const vaMuseum_1 = require("../services/vaMuseum");
const rijksmuseum_1 = require("../services/rijksmuseum");
const metService = metMuseum_1.MetMuseumService.getInstance();
const vaService = vaMuseum_1.VAMuseumService.getInstance();
const rijksService = rijksmuseum_1.RijksmuseumService.getInstance();
const searchArtworks = async (req, res) => {
    try {
        const { q, source = 'all', // 'met', 'va', or 'all'
        department, hasImages = 'true', isHighlight, limit = '20' } = req.query;
        const parsedLimit = limit ? Math.min(parseInt(limit), 200) : 200; // Max 200 results, no default
        let allArtworks = [];
        ...
        if (source === 'met' || source === 'all') {
            try {
                ...
                const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error('Met Museum search timed out')), 25000) // 25 second timeout
                );
...
const searchByTitleOrArtist = async (req, res) => {
    try {
        const { query, searchType = 'both', // 'title', 'artist', or 'both'
        source = 'all', hasImages = 'true', limit = '20' } = req.query;
        if (!query) {
        ...
        const parsedLimit = limit ? Math.min(parseInt(limit), 200) : 200;
        let allArtworks = [];
        // Search Met 
        if (source === 'met' || source === 'all') {
            try {
                ...
                const uniqueMetArtworks = metArtworks.filter((artwork, index, self) => index === self.findIndex(a => a.id === artwork.id));
        ...
        if (source === 'va' || source === 'all') {
            try {
                ...
                const filteredVAArtworks = vaArtworks.filter(artwork => {
                    const titleMatch = artwork.title && artwork.title.toLowerCase().includes(query.toLowerCase());
                    const artistMatch = artwork.artist && artwork.artist.toLowerCase().includes(query.toLowerCase());
                    if (searchType === 'title')
                        return titleMatch;
        ...
        const uniqueArtworks = allArtworks.filter((artwork, index, self) => index === self.findIndex(a => a.id === artwork.id));
        if (source === 'all') {
...
exports.searchByTitleOrArtist = searchByTitleOrArtist;
...
const getRandomArtworks = async (req, res) => {
    try {
        ...
        const parsedCount = count ? Math.min(parseInt(count), 50) : 50; // Max 50 random artworks, no default
        let allArtworks = [];
...
exports.getRandomArtworks = getRandomArtworks;
...
const searchMetMuseum = async (req, res) => {
    try {
        const { q, departmentId, hasImages = 'true', isHighlight, limit } = req.query;
        const artworks = await metService.searchStandardizedArtworks({
            q: q,
            departmentId: departmentId ? parseInt(departmentId) : undefined,
            hasImages: hasImages === 'true',
            isHighlight: isHighlight === 'true' ? true : undefined,
            limit: limit ? Math.min(parseInt(limit), 200) : 200
        });
        res.json({
...
exports.searchMetMuseum = searchMetMuseum;
const getMetArtwork = async (req, res) => {
    try {
...
exports.getMetArtwork = getMetArtwork;
// Rijksmuseum specific endpoints
const searchRijksmuseum = async (req, res) => {
    try {
        const { q, creator, type, technique, material, hasImages = 'true', limit = '20' } = req.query;
...
exports.searchRijksmuseum = searchRijksmuseum;
const getRijksArtwork = async (req, res) => {
    try {
        const { object_id } = req.params;
...
exports.getRijksArtwork = getRijksArtwork;
// Individual museum title/artist search endpoints
const searchMetByTitleOrArtist = async (req, res) => {
    try {
        const { query, searchType = 'both', hasImages = 'true', departmentId, limit } = req.query;
        if (!query) {
            return res.status(400).json({
...
exports.searchMetByTitleOrArtist = searchMetByTitleOrArtist;
const searchVAMuseum = async (req, res) => {
    try {
        const { q, images_exist = 'true', limit } = req.query;
        const parsedLimit = limit ? Math.min(parseInt(limit), 200) : 200;
...
exports.searchVAMuseum = searchVAMuseum;
const searchVAByTitleOrArtist = async (req, res) => {
    try {
        const { query, searchType = 'both', images_exist = 'true', limit } = req.query;
        if (!query) {
            return res.status(400).json({
                error: 'Query parameter is required'
            });
...
exports.searchVAByTitleOrArtist = searchVAByTitleOrArtist;
const getVAArtwork = async (req, res) => {
    try {
...
exports.getVAArtwork = getVAArtwork;
const searchRijksByTitleOrArtist = async (req, res) => {
    try {
        const { query, searchType = 'both', hasImages = 'true', type, limit = '20' } = req.query;
        if (!query) {
            return res.status(400).json({
                error: 'Query parameter is required'
            });
        ...
        if (searchType === 'title' || searchType === 'both') {
            try {
                const titleResults = await rijksService.searchStandardizedArtworks({
                    q: query, // In Rijksmuseum, the main 'q' parameter searches titles
                    imageAvailable: hasImages === 'true',
                    type: type,
                    limit: Math.ceil(parsedLimit / (searchType === 'both' ? 2 : 1))
                });
        ...
        if (searchType === 'artist' || searchType === 'both') {
            try {
                const artistResults = await rijksService.searchStandardizedArtworks({
                    creator: query, // Use creator parameter for artist search
                    imageAvailable: hasImages === 'true',
                    type: type,
                    limit: Math.ceil(parsedLimit / (searchType === 'both' ? 2 : 1))
                });
...
exports.searchRijksByTitleOrArtist = searchRijksByTitleOrArtist;
