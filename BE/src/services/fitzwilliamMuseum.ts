...
export class FitzwilliamMuseumService {
  private static instance: FitzwilliamMuseumService;
  private authToken: string | null = null;

  public static getInstance(): FitzwilliamMuseumService {
  ...
  async searchArtworks(params: {
    ...
    size?: number;
    sort?: string;
    images?: number; // 1 for objects with images only
    department?: string;
    type?: string;
    maker?: string;
    materials?: string;
    ...
    try {
      ...
      const response = await axios.get(`${FITZWILLIAM_BASE_URL}/objects`, {
        params: queryParams,
        headers
      });
   ...
   */
  async getArtworkByUuid(uuid: string): Promise<FitzwilliamArtwork> {
    try {
      const headers = await this.getHeaders();
      const response = await axios.get(`${FITZWILLIAM_BASE_URL}/objects/${uuid}`, {
        headers
      });
   ...
   */
  async getRandomArtworks(count: number = 20): Promise<FitzwilliamArtwork[]> {
    try {
  ...
  convertToStandardized(artwork: FitzwilliamArtwork): StandardizedArtwork {
    ...
    return {
      ...
      imageUrl,
      smallImageUrl,
      additionalImages: artwork.multimedia?.slice(1).map(img => 
        img.processed?.large?.location || 
        img.processed?.medium?.location || 
        img.processed?.original?.location
      ...
      tags: [
        ...(artwork.categories?.map(c => c.summary_title).filter((title): title is string => Boolean(title)) || []),
        ...(artwork.techniques?.map(t => t.summary_title).filter((title): title is string => Boolean(title)) || []),
        ...(artwork.materials?.map(m => m.summary_title).filter((title): title is string => Boolean(title)) || [])
      ],
      source: 'fitzwilliam',
  ...
  async searchStandardizedArtworks(params: {
    ...
    dateFrom?: string;
    dateTo?: string;
  }): Promise<{ artworks: StandardizedArtwork[]; total: number; page: number; totalPages: number }> {
    try {
      const searchResponse = await this.searchArtworks({
        ...params,
      ...
      const standardizedArtworks = searchResponse.data.map(artwork => 
        this.convertToStandardized(artwork)
      );
