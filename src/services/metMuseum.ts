...
export class MetMuseumService {
   ...
   */
  async getArtworkById(objectID: number, retryCount = 3): Promise<MetArtwork> {
    // Check if service is in cooldown
    if (this.isInCooldown()) {
      throw new Error(`Met Museum service temporarily unavailable due to rate limiting. Please try again later.`);
    }
    ...
    for (let attempt = 1; attempt <= retryCount; attempt++) {
      try {
        ...
        console.error(`Error fetching Met Museum artwork ${objectID} (attempt ${attempt}/${retryCount}):`, 
  ...
  async searchStandardizedArtworks(params: {
    ...
    return await Promise.race([
      ...
      new Promise<StandardizedArtwork[]>((_, reject) => 
        setTimeout(() => reject(new Error('Met Museum search timed out after 20 seconds')), 20000)
      )
    ]);
  }

  private async performSearchStandardizedArtworks(params: {
    q?: string;
    departmentId?: number;
    hasImages?: boolean;
    isHighlight?: boolean;
